...
const ProductPage = async ({
  ...
  const shareUrl = `${baseUrl}/products/${slug}`;

  return (
      <div>
        <Container className="flex flex-col md:flex-row gap-10 py-10">
          {product?.image && (
              <div className="w-full md:w-1/2 h-auto border border-pink-500/20 shadow-md rounded-md group overflow-hidden">
                <Image
          ...
          <div className="w-full md:w-1/2 flex flex-col gap-5">
            ...
            <div className="flex flex-wrap items-center justify-between gap-2.5 border-b border-b-gray-200 py-5 -mt-2">
              <div className="flex items-center gap-2 text-sm text-black hover:text-red-600 hoverEffect">
                ...
                <p><Link href={`/product/${slug}/reviews`}>View Reviews</Link></p>
              </div>
              <div className="flex items-center gap-2 text-sm text-black hover:text-red-600 hoverEffect">
                <FaRegQuestionCircle className="text-lg" />
                <p><Link href={"/contact"}>Ask a question</Link></p>
              </div>
              <div className="flex items-center gap-2 text-sm text-black hover:text-red-600 hoverEffect">
                <Heart className="text-lg" />
                <p><Link href={"/returns"}>Returns & Refunds</Link></p>
              </div>
              <ShareButton
                  title={product.name || ''}
            ...
            <div className="flex flex-wrap items-center gap-5">
              <div className="border border-pink-500/20 text-center p-3 hover:border-pink-500 hoverEffect rounded-md">
